  ...
  const controller: UsePaymentController = {
    ...
    handleInitializePaymentSheet: async (secret: string) => {
      try {
        ...
        if (error) {
          Alert.alert('Erro na Inicialização', `Código: ${error.code}\n${error.message}`);
    ...
    handleOpenPaymentSheet: async () => {
      // 🚫 Evento gratuito → nem abre o Stripe
      if (params.amount === 0) {
      ...
      if (!clientSecret || !isSheetInitialized) {
        Alert.alert('Aguarde', 'O formulário de pagamento ainda está sendo preparado.');
        return;
      }
      ...
      if (error) {
        if (error.code === 'Canceled') {
          ...
          Alert.alert('Erro no Pagamento', `Código: ${error.code}\n${error.message}`);
        }
      } else {
        await controller.handleCreateTickets();
      }
    ...
    handleCreateTickets: async () => {
      try {
        ...
        Alert.alert(
          '🎉 Ingressos Gerados!',
          `Você recebeu ${params.quantity} ingresso(s) para o evento "${params.eventName}".Eles já estão disponíveis na sua lista de ingressos.`,
          [
            {
              text: 'OK',
              onPress: () => {
