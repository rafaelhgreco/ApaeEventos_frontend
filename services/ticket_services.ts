...
export const createTicket = async (ticket: TicketData, token: string) => {
  try {
    const response = await axios.post(
      ...
      {
        // ✅ mesmo formato que o backend espera
    ...
    console.error("❌ Erro ao criar ticket:", error?.response?.data || error);
    throw error;
  }
};
...
export const listUserTickets = async (token: string) => {
  try {
    ...
    console.error("❌ Erro ao listar tickets:", error?.response?.data || error);
    throw error;
  }
};
...
export const getTicketById = async (id: number, token: string) => {
  try {
    ...
    console.error("❌ Erro ao buscar ticket:", error?.response?.data || error);
    throw error;
  }
};
